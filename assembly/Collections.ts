...
export class Collections {
  ...
  set_mint_price(args: collections.set_mint_price_arguments): collections.empty_object {
    // Check owner
    const config = this._state.getConfig();
    this._checkOwner(config);

    // Update MINT_PRICE constant
    Constants.MINT_PRICE = args.value;
  ...
  mint(args: collections.mint_arguments): collections.empty_object {
    ...
    let mintPrice: u64;
    if (supply.value < 300) {
      mintPrice = 7000000000; // 70 KOIN
    } else if (supply.value >= 300 && supply.value < 600) {
      mintPrice = 8500000000; // 85 KOIN
    } else if (supply.value >= 600) {
      mintPrice = 9500000000; // 95 KOIN
    } else {
      mintPrice = Constants.MINT_FEE ? Constants.MINT_PRICE : 0; // Use MINT_FEE if defined
    }

    // Pay mint price token or check creator
    if (mintPrice > 0) {
      const token_pay = new Token(Constants.TOKEN_PAY);
      const _result = token_pay.transfer(to, Constants.ADDRESS_PAY, SafeMath.mul(args.number_tokens_to_mint, mintPrice));
